export function generateDefaultSettings(name: string) {
    return {
        name: name,
        audioUpdateSettings: {
            tickers: ["SPY"],
...
export function validateSettingsObject(settings: any): boolean {
    ...
    const hasCorrectShape = (ticker:any) => !ticker.ticker ||
    ...
    if (settings.notificationUpdateSettings.tickers.length > 0) {
        if(!settings.notificationUpdateSettings.tickers.every(hasCorrectShape)) {
            return false;
        }
    }
